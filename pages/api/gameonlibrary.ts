...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    ...
    try {
      const gameInLibrary = await db.get('SELECT 1 FROM library WHERE user_id = ? AND title = ?', [userId, title]);
      const exists = !!gameInLibrary;

      res.status(200).json({ exists });
    } catch (error) {
      console.error('Failed to check game in library:', error);
